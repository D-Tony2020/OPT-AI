...
$$f(y) = f(x) + \nabla^{\top} f\left(\gamma x + (1-\gamma)y\right) \cdot (y - x) \quad \text{for some } \gamma \in (0, 1)$$
...
$f: \mathbb{R}^n \to \mathbb{R}$ 是凸的 $\iff$
...
**证明思路：** 反复使用凸函数的二点定义。将 $\sum_{\ell=1}^{k} \lambda^\ell x^\ell$ 写成 $\lambda^1 x^1 + (1-\lambda^1) \cdot [\text{其余的凸组合}]$，然后递归展开。
...
#### HW2 Q1: Gradient Descent with Bisection Line Search
...
#### HW3 Q1: Penalty Function Method
...
## constraint 2: g2(x) = x1^2 + x2^2 - 3 <= 0
def constraint2(x):
...
## penalized objective: f(x) + theta * max(0,g1)^2 + theta * max(0,g2)^2
def penalized_f(x, theta):
    val = function(x)
    val += theta * max(0, constraint1(x))**2
    val += theta * max(0, constraint2(x))**2
    return val

## gradient of penalized objective
def penalized_grad(x, theta):
    g = gradient(x).copy()
    c1 = constraint1(x)
    if c1 > 0:
        g += theta * 2 * c1 * grad_constraint1(x)
    c2 = constraint2(x)
    if c2 > 0:
        g += theta * 2 * c2 * grad_constraint2(x)
    return g

## bisection search
def bisection_search(f_line, a, b, eps=1e-4, tol=1e-5, max_iter=1000):
    k = 0
    while abs(b - a) > tol and k < max_iter:
        mid = (a + b) / 2
        lam, rho = mid - eps, mid + eps
        if f_line(lam) <= f_line(rho):
            b = rho
        else:
            a = lam
        k += 1
...
def penalty_method(x0, theta_list, gd_iters=500):
    x = np.array(x0, dtype=float)
    history = [x.copy()]  # track path for plotting

    for k, theta in enumerate(theta_list):
        # inner gradient descent loop
        for i in range(gd_iters):
            g = penalized_grad(x, theta)
            if np.linalg.norm(g) < 1e-8:
                break
            d = -g
            f_line = lambda alpha: penalized_f(x + alpha * d, theta)
            step = bisection_search(f_line, 0, 1.0)
            x = x + step * d

        history.append(x.copy())

        fval = function(x)
        pval = penalized_f(x, theta)
        c1, c2 = constraint1(x), constraint2(x)
        print(f'Penalty iter {k+1} | theta = {theta}')
        print(f' x = ({x[0]:.6f}, {x[1]:.6f})')
        print(f' f(x) = {fval:.6f}')
        print(f' penalized f(x) = {pval:.6f}')
        print(f'g1(x) = {c1:.6f},  g2(x) = {c2:.6f}')
        print()
...
Starting from $x^0 = (0, 0)$, $\theta \in \lbrace 1, 10, 100, 1000, 10000\rbrace$:

| Iter | $x_1$ | $x_2$ | $f(x)$ | $g_1(x)$ | $g_2(x)$ |
|------|--------|--------|---------|-----------|-----------|
| 1 | 0.274724 | 1.744780 | 13.627947 | 0.019503 | 0.119730 |
| 2 | 0.289955 | 1.711290 | 13.896839 | 0.001245 | 0.012587 |
| 3 | 0.292581 | 1.707531 | 13.925696 | 0.000112 | 0.001267 |
| 4 | 0.292862 | 1.707149 | 13.928608 | 0.000011 | 0.000127 |
| 5 | 0.292890 | 1.707111 | 13.928900 | 0.000001 | 0.000013 |
| 6 | 0.292893 | 1.707107 | 13.928929 | 0.000000 | 0.000001 |
...
![Penalty Method Convergence Path|581](assets/hw3_fig1.png)
...
![Excel Solver Setup and Results|597](assets/hw3_p5_img1.jpeg)
...
#### HW2 Q2: Proof that $h(x, y) = o(\lVert y - x \rVert)$
...
#### HW3 Q2(a): Show $f(x) = x^2$ is convex

We want to show $f(\lambda x + (1-\lambda)y) \leq \lambda f(x) + (1-\lambda)f(y)$ for all $x, y \in \mathbb{R}$, $\lambda \in [0,1]$.

We want to show:

$$(\lambda x + (1-\lambda)y)^2 \leq \lambda x^2 + (1-\lambda)y^2 \quad (\dagger)$$

Expand the left side:

$$\lambda^2 x^2 + (1-\lambda)^2 y^2 + 2\lambda(1-\lambda)xy \leq \lambda x^2 + (1-\lambda)y^2$$

Is it true that

$$2\lambda(1-\lambda)xy \leq x^2(\lambda - \lambda^2) + y^2((1-\lambda) - (1-\lambda)^2)$$

Note $\lambda - \lambda^2 = \lambda(1-\lambda)$ and $(1-\lambda) - (1-\lambda)^2 = (1-\lambda)\lambda$. So is it true that

$$2\lambda(1-\lambda)xy \leq \lambda(1-\lambda)x^2 + \lambda(1-\lambda)y^2$$

Cancel $\lambda(1-\lambda) \geq 0$:
...
#### HW3 Q2(b): Show $f(x) = (x - a)^2$ is convex

We want to show $f(\lambda x + (1-\lambda)y) \leq \lambda f(x) + (1-\lambda)f(y)$.

Is it true that:

$$(\lambda x + (1-\lambda)y - a)^2 \leq \lambda(x-a)^2 + (1-\lambda)(y-a)^2$$

Write $a = \lambda a + (1-\lambda)a$. Then the LHS becomes:

$$(\lambda(x-a) + (1-\lambda)(y-a))^2 \leq \lambda(x-a)^2 + (1-\lambda)(y-a)^2$$

They are the same after replacing $x$ and $y$ with $(x-a)$ and $(y-a)$. So do the same steps as in Part (a).

---

#### HW3 Q2(c): Show $f(\mathbf{x}) = \lVert \mathbf{x} - \mathbf{a} \rVert^2$ is convex

We have $f(x) = \sum_{i=1}^{n}(x_i - a_i)^2$.

We want to show: $f(\lambda x + (1-\lambda)y) \leq \lambda f(x) + (1-\lambda)f(y)$

Note that $\lambda x + (1-\lambda)y$ is the $n$-dimensional vector with $i$-th component $\lambda x_i + (1-\lambda)y_i$. So:

$$f(\lambda x + (1-\lambda)y) = \sum_{i=1}^{n}(\lambda x_i + (1-\lambda)y_i - a_i)^2$$

Also, $f(x) = \sum_{i=1}^{n}(x_i - a_i)^2$ and $f(y) = \sum_{i=1}^{n}(y_i - a_i)^2$.

We want to show:

$$\sum_{i=1}^{n}(\lambda x_i + (1-\lambda)y_i - a_i)^2 \leq \sum_{i=1}^{n}(\lambda(x_i - a_i)^2 + (1-\lambda)(y_i - a_i)^2)$$

By Part (b), we know

$$(\lambda x_i + (1-\lambda)y_i - a_i)^2 \leq \lambda(x_i - a_i)^2 + (1-\lambda)(y_i - a_i)^2 \quad \text{for all } i = 1,\ldots,n.$$

Thus, adding these inequalities for $i = 1,\ldots,n$, we get

$$\sum_{i=1}^{n}(\lambda x_i + (1-\lambda)y_i - a_i)^2 \leq \sum_{i=1}^{n}(\lambda(x_i - a_i)^2 + (1-\lambda)(y_i - a_i)^2)$$

---

#### HW2 Q3: Convex Combination (Jensen's Inequality for 4 Points)
...
#### HW2 Q4: Positive Combination of Convex Functions is Convex
...
#### HW3 Q3: Strongly Convex GD Convergence Bound

We want to show: if $f$ is strongly convex with parameter $\mu$, then

$$\lVert x^{k+1} - x^{\ast} \rVert^2 \leq (1-\mu\gamma)\lVert x^k - x^{\ast} \rVert^2 + 2\gamma(f(x^{\ast}) - f(x^k)) + \gamma^2\lVert \nabla f(x^k) \rVert^2.$$

**Proof.** Start with the definition of strong convexity, setting $x = x^k$ and $y = x^{\ast}$:

$$f(x^{\ast}) \geq f(x^k) + \nabla^{t} f(x^k)(x^{\ast} - x^k) + \frac{\mu}{2}\lVert x^k - x^{\ast} \rVert^2$$

From gradient descent, $x^{k+1} = x^k - \gamma\nabla f(x^k)$, so $\nabla f(x^k) = \frac{1}{\gamma}(x^k - x^{k+1})$. Substituting:

$$f(x^{\ast}) \geq f(x^k) + \frac{1}{\gamma}(x^k - x^{k+1})^{t}(x^{\ast} - x^k) + \frac{\mu}{2}\lVert x^k - x^{\ast} \rVert^2$$

Using the identity $x^{t} y = \frac{1}{2}(\lVert x+y \rVert^2 - \lVert x \rVert^2 - \lVert y \rVert^2)$ with $x = x^k - x^{k+1}$ and $y = x^{\ast} - x^k$:

$$f(x^{\ast}) \geq f(x^k) + \frac{1}{2\gamma}(\lVert x^{\ast} - x^{k+1} \rVert^2 - \lVert x^k - x^{k+1} \rVert^2 - \lVert x^{\ast} - x^k \rVert^2) + \frac{\mu}{2}\lVert x^k - x^{\ast} \rVert^2$$

Multiply both sides by $2\gamma$:

$$2\gamma(f(x^{\ast}) - f(x^k)) \geq \lVert x^{\ast} - x^{k+1} \rVert^2 - \gamma^2\lVert \nabla f(x^k) \rVert^2 - \lVert x^{\ast} - x^k \rVert^2 + \mu\gamma\lVert x^k - x^{\ast} \rVert^2$$

where we used $\lVert x^k - x^{k+1} \rVert^2 = \gamma^2\lVert \nabla f(x^k) \rVert^2$. Rearranging:

$$\lVert x^{k+1} - x^{\ast} \rVert^2 \leq 2\gamma(f(x^{\ast}) - f(x^k)) + \gamma^2\lVert \nabla f(x^k) \rVert^2 + (1-\mu\gamma)\lVert x^k - x^{\ast} \rVert^2$$

---

### Appendix — Not in 9 Exam Topics

#### HW1 Q3: Logistic Regression Gradient
